...
Status: required coordinated release gate. This test is independent of TmuxDeck and must pass before any Pi, Claude, Codex, OpenCode, or Orchestrator protocol-v4 candidate is published.
...
The executable cross-package test runner lives in its own integration repository/workspace, not in TmuxDeck and not inside one adapter package. It consumes packed candidate tarballs and the canonical `@dataforxyz/agent-intercom-core/protocol-v4` acceptance vectors/hash.
...
- Orchestrator manager and workers
...
Use a pairwise cross-harness routing matrix so every adapter sends and receives through another adapter, not only through Pi.
...
- built-in Pi, Claude, Codex, and OpenCode workers inherit the exact scope privately
